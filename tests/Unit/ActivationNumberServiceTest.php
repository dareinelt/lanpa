...
Runner::test('Rufnummer wird angelegt und eindeutig validiert', static function (): void {
    ...
    $id = $service->create(['phone' => '+49 170 1234567', 'active' => true]);

    $item = $service->find($id);
    Assert::same('491701234567', (string) $item['phone_digits']);

    try {
        $service->create(['phone' => '+49 170 1234567', 'active' => true]);
        Assert::true(false);
...
Runner::test('Ungültige Rufnummer wird abgelehnt', static function (): void {
    $pdo = activationNumberPdo();
    $service = activationNumberService($pdo);

    try {
        $service->create(['phone' => '', 'active' => true]);
        Assert::true(false);
    } catch (ValidationException $exception) {
        Assert::true(isset($exception->errors()['phone']));
    }
